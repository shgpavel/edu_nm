...
struct r_data richardson(quad_func qf, double eps, size_t initial_steps) {
  ...
  struct r_data *report = (struct r_data*)malloc(10 * sizeof(struct r_data));
	...
	for (int iter = 0; iter < max_iterations; ++iter) {
		...
		struct r_data rich_res = richardson_step(m_int, &sums, sums.size, first_step);
		...
		if (report_len >= report_cap) {
			report_cap *= 2;
			report = (struct r_data*)realloc(report, report_cap * sizeof(struct r_data));
		}
...
struct r_data step_guessing(quad_func qf, double eps) {
	...
	struct r_data first_rep =
		{.error = nan(""), .step = B - A, .m = nan(""), .value = vector_val(&sums, 0)};
	struct r_data sec_rep =
		{.error = nan(""), .step = (B - A) / 2, .m = nan(""), .value = vector_val(&sums, 1)};
	...
	struct r_data thr_rep = richardson_step((int)m, &sums, sums.size, B - A);
	struct r_data fr_rep =
		{.error = thr_rep.error, .step = (B - A) / 4, .m = thr_rep.m, .value = vector_val(&sums, 4)};
