...
int get_data_tonethree() {
	...
	err = fprintf(csvout, "n, D_left(n), D_mid(n), D_trapz(n), D_simpson(n), "
	...
	for (size_t i = 1; i <= 100; ++i) {
		...
		err = fprintf(csvout, "%zu, %.15lf, %.15lf, %.15lf, %.15lf, %.15lf, %.15lf\n",
						i, absdiff[0], absdiff[1], absdiff[2], absdiff[3],
						absdiff[4], absdiff[5]);
		if (err < 0) {
			return -3;
		}
...
int get_data_pentwo() {
	...
	for (size_t i = 1; i <= 30; ++i) {
		...
		double absdiff_ncg = fabs(val2 - newton_cotes_nxn(2 * i + 1, A, B));
		
		err = fprintf(csvout, "%zu, %.15lf, %.15lf\n",
									i, absdiff_ncc, absdiff_ncg);
		if (err < 0) {
			return -3;
		}
	}
...
int compound_extra_penthree() {
	...
	for (size_t l = 1; l <= 50; ++l) {
		double sval = compound_ncwgau(l, 3, newton_cotes_nxn, gauss_quad);
	...
	for (size_t l = 1; l <= 50; ++l) {
		double sval = compound_ncwgau(l, 6, newton_cotes_nxn, gauss_quad);
...
int look_basic_funcs_penfive() {
	...
	printf("S = %.7lf e = %e m = %.7lf on step = %e\n",
				 res3.value, res3.error, res3.m, res3.step);
	printf(";;\n\n");

	printf(";; pen.5 Simpson\n");
	struct r_data res4 = richardson(quad_simpson, eps, 1);
	printf("S = %.7lf e = %e m = %.7lf on step = %e\n",
				 res4.value, res4.error, res4.m, res4.step);
	printf(";;\n\n");
	return 0;
}
